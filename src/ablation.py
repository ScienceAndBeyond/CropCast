...
This isolates changes by source. Rows, model, hyperparameters and seeds are held fixed;
...
The metric is the RMSE ratio Q = RMSE(ablated) / RMSE(intact). The primary
equivalence margin is [0.95, 1.05], and an interval spanning the margin is
inconclusive rather than evidence of equivalence.
...
# Primary margin first; secondary margins are reported for sensitivity only.
MARGIN = 0.05
...
def main() -> None:
    ...
    log.info(f"  primary margin [{1-MARGIN:.2f}, {1+MARGIN:.2f}], "
    ...
    log.info("  PER-YEAR Q (pooled results can hide opposing years)")
