"""
Train county-level crop yield models and write the main result tables.

Models are fitted with climate-only, climate+soil, climate+vegetation, and full
feature sets. Results include raw test R2 and skill relative to each county's
training-period mean, since county identity explains a large share of yield
variation. Interpretation outputs are produced for the climate+soil model; the
vegetation indices are treated as same-season predictors, not causal inputs.

Outputs include model performance, feature importance, ALE/PDP summaries,
collinearity checks, sensitivity summaries, and improvement tables.
"""
...
def filter_by_year(df: pd.DataFrame, min_year: int, max_year: int = None) -> pd.DataFrame:
    ...
    max_year is enforced as well as min_year so the data stays within the
    configured analysis window.
    """
...
def merge_datasets(yield_df: pd.DataFrame, climate_df: pd.DataFrame,
    ...
    # NASS can publish the same county-year-crop as both aggregate and
    # irrigated/non-irrigated rows. Keep those modes separate so counts and
    # scores are not multiplied.
    ...
    # climate.csv also carries n_months. Exclude it so the vegetation
    # completeness column keeps its expected name.
    ...
    # Keep vegetation n_months under a unique name for completeness filtering.
    ...
    # Warn if any source stops short of the configured analysis window.
...
def build_study_baseline(
    ...
    # comparable with the same statistics over a full one.
...
def county_mean_baseline(train_df: pd.DataFrame, test_df: pd.DataFrame) -> np.ndarray:
    """Predict each county's training mean, with a global fallback."""
...
def train_model(X_train, y_train, X_test, y_test, features: List[str],
    ...
    """Fit one Random Forest per seed and average the scores."""
    ...
    for seed in SEEDS:
        ...
        # Skill relative to the county-mean baseline.
...
def spatial_cv_r2(crop_df: pd.DataFrame, features: List[str], n_splits: int = 5) -> Optional[float]:
    """Mean R2 across county-grouped folds."""
...
def compute_ale_data(model, train_df: pd.DataFrame, crop: str,
    ...
    """
    Compute 1D Accumulated Local Effects for the most important features.

    ALE is less prone than PDP to evaluating impossible feature combinations
...
def compute_sensitivity_from_pdp(pdp_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (crop, feature), group in pdp_df.groupby(["crop", "feature"]):
        # Sort by feature value before reading the direction.
    ...
    if perf_df is not None:
        for crop in perf_df["crop"].unique():
            ...
            if len(baseline) > 0:
                ...
                # Keep a legitimate R2 of exactly 0.0 instead of treating it as missing.
