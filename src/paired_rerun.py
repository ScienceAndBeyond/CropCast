"""
Compare archived and current data on matched county-years.

The current pipeline keeps more rows than the archived one, so this script first
intersects the datasets on crop, county, and year. It then trains the same model
on the same rows using archived features and current features in turn. Per-year
SSE is also reported because pooled scores can hide year-level differences.

Run from src/:  python -X utf8 paired_rerun.py
Writes ../results_comparison/.
"""
...
def main() -> None:
    ...
    log.info("  Interpretation: the current dataset predicts these same county-years")
