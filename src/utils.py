...
def save_df(df: pd.DataFrame, file_name, mode: str = "w") -> None:
    ...
    if extn == ".csv":
        if mode == "a" and file_path.exists():
            # Align appended rows to the existing header. pandas writes CSV
            # columns in DataFrame order, so mismatched order can put valid
            # values under the wrong column names.
