"""
Paired irrigated-vs-non-irrigated corn weather sensitivity.

NASS reports irrigated and non-irrigated county corn yields separately for some
county-years. Where both practices are reported in the same county and year,
this script compares each practice's yield anomaly against the same weather
anomaly record.

The results are within-county associations, not causal treatment effects. The
paired design reduces between-county geography differences, but the irrigated
and non-irrigated acres may still differ within a county. Linear detrending,
leave-one-year-out refits, state-specific fits, and equal-state weighting are
reported as sensitivity checks.

Outputs (RESULTS_DIR):
  irrigation_contrast.csv
  irrigation_variance_explained.csv
  irrigation_leave_one_year_out.csv
  irrigation_by_state.csv
  irrigation_state_balanced.csv
  irrigation_pairs.csv
...
def load_pairs() -> pd.DataFrame:
    ...
    # Match the main model's complete-season climate rule before fitting slopes.
    n0 = len(df)
...
def recompute_anomalies(df: pd.DataFrame) -> pd.DataFrame:
    """Derive county anomalies and linear-detrended anomalies."""
    df = df.copy()
...
def contrast(df: pd.DataFrame, suffix: str, seed: int = RANDOM_SEED) -> pd.DataFrame:
    """Single-weather slopes per practice with county-clustered intervals."""
    ...
    for w in WEATHER:
        ...
        # One-sided check on absolute slope magnitude.
...
def state_balanced_variance_explained(df: pd.DataFrame, suffix: str) -> pd.DataFrame:
    """Same fit as variance_explained(), but reweighted so KS and NE contribute
    equal total weight, rather than by row count.

    Nebraska supplies 665 of 867 pairs (77%) and dominates the pooled result by
    sheer count. Row-weighting each state's pairs to 1.0 / n_rows_in_state
    answers "does the finding depend on Nebraska's larger sample", not "what
    is the true population effect" - there is no reason to think KS and NE
    are equally representative of anything. Report alongside the pooled
    figure, never in place of it.
    """
    if df["state_alpha"].nunique() < 2:
        return pd.DataFrame()
    counts = df["state_alpha"].value_counts()
    w = (1.0 / df["state_alpha"].map(counts)).to_numpy()
    w = w / w.sum() * len(df)  # normalise so weighted N matches actual N

    X = np.column_stack([df[c + suffix].to_numpy() for c in WEATHER] + [np.ones(len(df))])
    out = []
    for p in PRACTICES:
        y = df[p + suffix].to_numpy()
        sw = np.sqrt(w)
        ...
        r2 = 1 - np.sum(w * (y - pred) ** 2) / ss_tot if ss_tot > 0 else np.nan
        out.append({"trend": TREND_MODES[suffix], "practice": p,
                     "r2_weather_state_balanced": round(float(r2), 4)})
    return pd.DataFrame(out)
...
def run() -> None:
    ...
    sb = pd.concat([state_balanced_variance_explained(df, s) for s in TREND_MODES],
                   ignore_index=True)
    ...
    for label in TREND_MODES.values():
        ...
        if not sb.empty:
            for _, r in sb[sb.trend == label].iterrows():
                logging.info(f"    {r['practice']:14s} weather R2  state-balanced (KS=NE weight)"
                             f"={r['r2_weather_state_balanced']:.4f}  <- NOT the pooled figure,"
                             f" answers whether NE's larger sample drives the result")
    ...
    save_df(sb, RESULTS_DIR / "irrigation_state_balanced.csv")
    save_df(df, RESULTS_DIR / "irrigation_pairs.csv")
    logging.info(f"\n  Written to {RESULTS_DIR}")
    logging.info("  Interpretation: within-county associations, in-sample. 2008-2018, corn, "
