...
# is what makes an associational within-county comparison possible for the
# central claim (irrigation_contrast.py): irrigated corn's yield anomaly is
# less explained by growing-season weather than rainfed corn's is. This is NOT
# a natural experiment or a randomized comparison - counties are not assigned
# to irrigate, and the paired sample requires >=4 years of both practices
# being reported, which selects for a specific kind of county (see
# irrigation_contrast.py's docstring and the README's irrigation section for
# the associational framing and its limits).
#
...
def get_required_months(slack: int = 1) -> List[int]:
    ...
    Deriving it here means widening a configured season also widens the
    downloaded month window.

    Winter crops are excluded upstream and would need separate two-year season
    handling.
    """
...
#
#   "both"       Everything, flagged. Rows duplicate by design; callers filter
#                on the `irrigation` column themselves. For inspection.
...
def resolve_states(states=None, study: bool = False) -> list:
    ...
    rather than requiring the list to be retyped on the command line.
...
def yield_filename(mode: str = None) -> str:
    ...
    Aggregate and split analyses use different units, so they get separate
    files and results directories.
    """
...
def merged_filename(mode: str = None) -> str:
    ...
    Keep this mode-specific so aggregate and split runs cannot overwrite each
    other's merged table.
    """
    mode = (mode or NASS_PRACTICE_MODE).lower()
