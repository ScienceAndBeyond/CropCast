...
#
# Notes:
# - Heavy work is server-side in Earth Engine.
...
# -----------------------------------------------------------------------------
# Climate feature names used by the model.
# -----------------------------------------------------------------------------
...
# Heat indices derived from daily maximum temperature only. These are Tmax-based
# exposure rates, not conventional growing degree days.
...
def build_monthly_climate_image(year: int, month: int) -> ee.Image:
    ...
    A spot check found image counts matched calendar days in six sampled
    ...
    Monthly rows keep the season definition as an analysis choice rather than a
    download choice. Vegetation is already monthly, so this matches that shape.
    ...
    Percentile bands are omitted because season percentiles cannot be recovered
    from monthly percentiles.
    """
    ...
    # Hottest month's county-average daily maximum.
...
def build_year_stack(year: int) -> ee.Image:
    ...
    Stacking months into bands keeps each state-year to one reduceRegions call.
    Select before rename so column order stays explicit.
    """
    imgs = []
...
def aggregate_to_season(monthly_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # A month with a missing value should not contribute its days to that
    # variable's denominator.
    w = df["N_DAYS"]

    # Days-weighted means
    # Each accumulating variable gets its own observed-days denominator.
    # min_count=1 keeps all-missing groups as NaN instead of zero.
    ...
    for c in accumulating:
        df["_d_" + c] = w.where(df[c].notna())
    # TMAX_MAX aggregates by max, but still needs coverage tracking.
...
def run(
    ...
    # When outputs are cleared, restart the checkpoint state as well.
    completed = load_checkpoint(CHECKPOINT_FILE) if (resume and not force) else set()

    # Fresh runs clear both outputs so appended chunk files cannot mix runs.
