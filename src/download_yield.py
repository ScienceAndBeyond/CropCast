...
def get_usda_crop_yield(
    ...
    if failed_states:
        # Report failed states at the end so partial downloads are visible.
        logging.error("=" * 70)
...
def clean_yield_data(df: pd.DataFrame, practice_mode: str = None) -> pd.DataFrame:
    ...
    # Tidy irrigation label. Unmapped labels fall back to a slug.
...
def main():
    ...
    if args.reuse_raw:
        ...
        # --states controls what gets fetched, not what is kept from cache.
        # ml.py applies the study-state filter later.
        ...
        # Refresh fetched states while keeping cached rows for other states.
        if raw_path.exists() and not raw_df.empty:
            existing = pd.read_csv(raw_path, low_memory=False)
            # Replace only states with successful new data.
