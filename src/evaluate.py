"""
Evaluate county-level crop yield models against stronger baselines.

County-year panels mix two kinds of variation: persistent differences between
counties and year-to-year movement within the same county. Raw R2 can be high
when a model mainly learns which counties usually yield more. This module adds
county-mean baselines, optional detrending, temporal/leave-year-out/spatial
splits, and a simple county-constant placebo for soil.

Outputs (RESULTS_DIR):
  evaluation_protocol.csv
...
def build_predictor_sets(df: pd.DataFrame) -> Dict[str, Optional[List[str]]]:
    """Ordered ladder from simple baselines to the full model."""
    def avail(fs):
        """Resolve a feature list and fail if any expected column is absent."""
    ...
    return {
        ...
        # Climate plus random county-constant features, matching climate_soil
        # except that soil columns are replaced by noise.
...
def add_placebo_features(df: pd.DataFrame, seed: int = RANDOM_SEED) -> pd.DataFrame:
    """
    Attach random county-constant features with no agronomic content.

    Random numbers have no geographic pattern, but real soil does (nearby
    counties tend to be similar). So "soil beats this control" is a real
    result, but it is not proof of how much of soil's value is agronomy
    versus just marking which county a row is from.
    """
    df = df.copy()
...
def predict_baseline(kind: str, train: pd.DataFrame, test: pd.DataFrame) -> np.ndarray:
    ...
    if kind == "county_mean_trend":
        # County level plus a single shared linear trend, estimated within
        # counties so an unbalanced panel does not bias the slope.
...
def score(y_true: np.ndarray, y_pred: np.ndarray, county_ref: np.ndarray) -> Dict[str, float]:
    """
    Three related scores with different baselines.

    r2            Conventional R2 against the test-set mean.

    skill_vs_county_mean
                  1 - SSE(model) / SSE(county-mean predictor).
                  0 means tied with the county mean, negative means worse.
        ...
        for scheme in schemes:
            ...
            for name, scores in per_set.items():
                ...
                rows.append({
                    ...
                    # Useful diagnostic alongside skill_vs_county_mean.
...
def run(crops: Optional[List[str]] = None, schemes: Optional[List[str]] = None,
    ...
    # Reapply the same completeness filters used by ml.py before scoring.
    ...
    for crop in res["crop"].unique():
        ...
        pl.append({
            ...
            # Share of the soil gain matched by this random county-constant
            # placebo. This is a diagnostic, not a decomposition.
            "pct_gain_matched_by_placebo": (
